...
int main(int argc, char *argv[])
{
    ...
    // Here we take port as arguments
    ...
    // Create a socket
    int new_socket_server = socket(AF_INET, SOCK_STREAM, 0);
    if (new_socket_server == -1)
    {
        perror("Error creating the socket server\n");
        WriteLog("Error creating the socket server\n");
    }
    if (bind(new_socket_server, (struct sockaddr *)&adrs, sizeof(adrs)) == -1)
    {
    ...
    if (listen(new_socket_server, 1) == -1)
    {
    ...
    int LenghtAdrs = sizeof(adrs);
    int CliSock = accept(new_socket_server, (struct sockaddr *)&adrs, (socklen_t *)&LenghtAdrs);
    if (CliSock == -1)
    {
    ...
    while (TRUE)
    {
        ...
        else if (read(CliSock, &cmd, sizeof(cmd)) > 0)
        { // If it receives a command by a client, the circle moves...
